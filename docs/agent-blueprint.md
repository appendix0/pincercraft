...
> **Status**: Plan committed 2026-05-20. **Supersedes** the original blueprint (preserved as Appendix A).
...
See [diagnosis-2026-05-20.md](./diagnosis-2026-05-20.md). Three-bullet recap:

- The orchestrator is still a `for (i=0; i<Infinity; i++)` loop in `agent.js:_processInput` that re-invokes the LLM on every system-message append. This is the **burst engine**.
- Every command is text-parsed (`!cmd(args)` regex), so there's no schema layer to reject hallucinated calls. Today's 70B emitted three structurally invalid commands in five turns.
- Long-running tools (`!newAction`, `!collectBlocks`, `!followPlayer`) block the agent loop. Mode interrupts during a running tool crash the process (today's `self_defense`-vs-`newAction` exit code 1).
...
**When:** at least one stable week running v2 by default on both engines. No exit-code-1 disconnects, no burst-related 429s observed.
...
1. **Tool count vs context size.** 60+ tools × ~150 tokens of schema each = ~9K tokens of system prompt overhead. Two-pronged response: (a) consolidate overlapping primitives in Step 2 — e.g. `!goToPlayer`/`!goToCoordinates`/`!followPlayer`/`!stay` → one `move({target, mode})`; (b) gate advertisement by context in Step 3 — long-running tools hidden when a bg handle is active; build/destroy tools hidden in plan mode for read-only players.

2. **Tolerant parsing for weak tool-use providers.** NVIDIA llama-3.3-70b sometimes emits malformed JSON in `tool_calls.function.arguments`. Step 3 includes one structured re-prompt with the parse error in context; beyond that we surface "brain disconnected" as today. Need a real metric in production for how often this fires.

3. **Streaming.** Defer to post-Step-7. Non-streaming first; streaming added once v2 is stable. Bedrock chat UX currently doesn't show partial responses anyway.

4. **Mode-trigger semantics in v2.** Modes (`self_defense`, `cowardice`, `hunting`) today inject side-effects on Mineflayer. In v2, modes can cancel the current bg handle and emit a `mode_trigger` event that re-engages the LLM with context ("You were attacked by a zombie while collecting cherry logs — defend or flee?"). Need to map each existing mode onto this contract.

5. **Bedrock-iPad slash skill UX.** `/init`/`/explore` etc. are reserved by Bedrock client. Workaround: prefix with `.` or restrict to whisper-only. Decide before Step 7.

6. **Subagent crash isolation.** If a subagent's internal loop crashes (exit code 1 from a Mineflayer bug), the parent should still receive a clean `{status: 'failed'}` and not propagate the crash. Need a per-subagent process boundary or strong try/catch.
