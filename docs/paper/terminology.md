...
| "a **stock** agent" for the ablated arm | **the unharnessed agent**, or "the same agent with the harness off" | `no harness` is this fork with every layer switched off, **not upstream Mindcraft** (results.md, limitation 6). The design's whole value is that only the harness varies; "stock" claims something about upstream that no arm measures. |
| "the harness improves success" as the headline | "the unharnessed agent overstates its own success by N points", N = the **false-completion rate** of the ablated arm (confirmatory: 45.1%) | The full harness's near-zero false-completion rate is partly by construction — `verify` blocks unearned finishes by design. Lead with the size of the failure in the ablated arm. Take N from the per-attempt form (§5.2), not the gap. |
