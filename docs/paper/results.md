...
**Draft, 2026-08-19.** Every number below is regenerated by
`python3 eval/campaign_report.py`; nothing here is transcribed by hand except
where a script under `eval/` is named (§2 comes from `eval/agreement.py`,
§8.1.1 from `eval/stock_strip_report.py`). Terms follow
...
Regenerated by `python3 eval/agreement.py report` (Wilson and κ from the same
rows).

| set | agreement | Wilson 95% | κ |
|---|---|---|---|
| **confirmatory, scorer-measured** | **67/68 = 98.5%** | **[92.1, 99.7]** | **0.970** |
...
**Gate requires ≥ 90% agreement and a Wilson lower bound > 75%. PASS** at 98.5%
and 92.1%. The pilot is reported separately and never pooled, per §5.

**Which set a label belongs to is a property of when the label was recorded**,
not of the attempt it judges (freeze date `2026-08-07`, `analysis_rules.FREEZE`).
The pilot rows are pilot because they developed the `end_factor` grammar they
test — that is what §5 forbids pooling. The confirmatory cell accordingly holds
two blind rounds: 39 labels recorded 2026-08-08 and 29 recorded 2026-08-16, the
latter drawn from the confirmatory campaign itself.

*This table read 39/39 = 100.0% [91.0, 100.0] until 2026-08-19, when it was
regenerated after the second round. The gate passes under either reading, and
also under the narrower cut of confirmatory-campaign attempts alone
(28/29 = 96.6%, [82.8, 99.4]).*

**Both confirmatory disagreements are the same task.** The one scorer-measured
miss is #1043 and the one honor-system miss is #384 — both the 5×5 platform, the
only structure-scored criterion in the set. **Every inventory-delta criterion
agreed, in both rounds.** So the instrument is calibrated tightly on the
criterion class carrying 12 of the 13 tasks and demonstrably weaker on the one
that is not — which is §11.2, and why §10 reports the campaign with that task
dropped. Dropping it makes both headline effects *larger*, so keeping it is the
conservative choice.
...
2. **The calibration set is an easy regime.** 67/68 is a strong lower bound
   (92.1%) but the attempts it covers are overwhelmingly inventory-delta criteria
   where ground truth is cheap. It does not license the scorer on harder
   criteria, and the evidence for that is inside the calibration itself: both
   confirmatory disagreements — #1043 scorer-measured and #384 honor-system —
   are the platform task, the only structure-scored criterion (§2).
