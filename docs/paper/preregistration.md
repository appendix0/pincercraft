...
| 2026-08-16 | **Pre-declaration of a stratified re-calibration of the scorer.** A second calibration set of **30 blind labels**, drawn from the confirmatory primary set by the rule below, reported as a **2×2 of scorer verdict against human verdict** rather than as one agreement percentage, and **never pooled** with the existing 39. **Universe:** the 356-row confirmatory primary set, minus `task_id` 1074, which maps to two `task_attempts` rows — `agreement.py` binds a label to the latest row for a `task_id`, so a label on a collided id would certify an attempt the labeller never saw. 355 eligible; none carries a human label. **Strata and allocation, fixed before the draw:** cell **B** (scorer says fail, agent claimed done — *false completion*, the endpoint every headline rests on) 39 eligible → **21 drawn**, split **17 inventory-delta / 4 block-scan**; cell **C** (scorer says fail, agent did not claim) 113 → **5**, split 4 / 1; cell **A** (scorer says success) 203 → **4**, all inventory-delta. **Draw:** `random.Random(20260816).sample(sorted(task_ids), k)` per stratum, in `eval/calib_sample.py`, emitting a manifest committed before the first card is rendered. Card rendering is unchanged (`evidence.py card()`), so the new labels are comparable to the old: the scorer's verdict and the arm are omitted, the task, criterion, tier, duration, the agent's claim and the inventory delta are shown. | **Post-data as to the campaign, pre-data as to these labels** — no card had been rendered and no label recorded when this entry was written. The existing H0 figure, 39/39 with Wilson [91.0, 100.0], is a **prevalence-weighted aggregate over a convenience sample**: cards were issued in glob order, and 34 of the 39 labels are *true completions*. The scorer's **false-completion** verdict — the exact judgement H1, H3, §8.1 and every headline rate depend on — is human-validated on **2 attempts**, Wilson [34.2, 100.0]. Aggregate agreement is the wrong statistic for a rare-cell endpoint; sensitivity and specificity stratified by scorer verdict are the right ones. Two further gaps close here: **no human label touches any `conf_*` attempt** (all 56 are on `play`/`bench_*` rows recorded on or before 2026-08-07, and "confirmatory" in `agreement.py` has meant *when the label was written*, not *what it judges*), and the **block-scan path added at `59308ba`/`e06b3c7` scores 28 primary rows with no blind label at all** — 11 of them in cell B, so 28% of the critical cell rests on an instrument no human has checked. **Pre-commitment on what a disagreement means, fixed while blind.** Per-cell agreement is reported with Wilson intervals and the cells are never summed, because the sample is deliberately non-representative and a pooled figure over it would be less honest than the one it replaces. If cell B agreement falls **below 90%**, the scorer over-calls false completion: the measured false-positive rate is applied as a correction to every reported false-completion rate, the corrected figures become the paper's numbers, and the result is written as a corrected instrument — not as a limitation of the labels, not as a reason to re-draw, and not as grounds to fall back on the 39/39. The same rule applies to cell A (a disagreement there corrects the success rates) and to cell C. **Abstentions are recorded as `null`, reported as abstentions, and counted as neither agreement nor disagreement** — the 2026-08-08 pass recorded the owner abstaining on all three blind platform cards, so a block-scan abstention is an expected outcome and is itself the finding for that path. **No card is re-drawn and no label is revised after the scorer's verdict is revealed**; the draw is deterministic from the declared seed, so the manifest is verifiable against this rule rather than trusted. |
| 2026-08-16 | **Three reporting corrections. No endpoint, interval, hypothesis or verdict changes.** (1) The bootstrap resolution floor is `2/(B+1)`, not `1/(B+1)`, and prints as **`p ≤ 0.0002`** rather than `p < 0.0001` — affects the H1 and H2 headline p-values in results.md §4 and §5. (2) `stock_strip_report.py` printed its Fisher exact p with `%.5f`, flattening **3.7 × 10⁻⁸ to `0.00000`**; it now prints scientific notation below 0.0001, and results.md §8.1.1 and the §10 position-matched table carry the exact values (3.7 × 10⁻⁸, 7.4 × 10⁻⁷, 5.7 × 10⁻⁷) in place of `< 0.0001`. (3) results.md §8.1's *"Give-tasks (`−N item`) are excluded"* is **factually wrong about this task set** — there are no give-tasks in it; all 13 criteria are `+N` deltas or the platform structure. The excluded row is the single non-delta **platform** attempt, for which "already held ≥ N of the target item" is undefined. The 22 denominator was always correct. | **Post-data reporting corrections, declared as such**, found in the external audit of 2026-08-16. All three are the same class of defect and two of them are *recurrences of one this project already fixed*: the 2026-08-10 deviation corrected `p=0.0000` by adding the `(r+1)/(B+1)` correction to the estimator, but left the resolution bound printed beside it at half the attainable minimum — so the report claimed the statistic had gone below a value it had in fact just computed — and left a second p-value in a different script rounding a real result to zero. A p-value floor is a claim about what the design bought; stating it tighter than the procedure allows overstates the evidence for free, on the two headline hypotheses. The give-task sentence is the inverse error: a correct number carrying a false justification, which is worse than a wrong number, because a reader checking the reason finds nothing wrong with the arithmetic and concludes the rule exists. **Direction of effect: none.** The corrected p-values are larger (weaker) than printed and the corrected Fisher values are far smaller (stronger) than `< 0.0001`; the denominator is unchanged. |
